...
int maxSteps = 0;                  //max number of steps the motor turns
...
void loop() 
{
  ...
  { 
    ...
    if (sensorState == 0)             //sensor beam is broken (LOW)
    {
      ...
      //StepperMotorReward();         //no reward is dispensed for South response
  ...
  else  //if incomingByte is not 'X' or 'S'...turn off LED
  {
      digitalWrite(LEDPIN, LOW);
  }  
}
