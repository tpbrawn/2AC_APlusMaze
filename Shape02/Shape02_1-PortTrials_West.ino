...
int maxSteps = 2500;               //max number of steps the motor turns
...
int beamBreakThreshold = 4;     //number of consecutive beam breaks needed to trigger response 
...
void loop() 
{
  ...
  else if (incomingByte == 'W')    //'W' sent from python to activate West response port   
  {
  ...
  {
      digitalWrite(LEDPIN, LOW);  //if incomingByte is not 'X' or 'W'...turn off LED
  }      
}
