...
#progresses, decrease the N/S reward and increase the E/W reward until
#rat is successful with no reward at N/S ports.

############################################################################
#run concurrently with arduino files:
#       -Shape03_2-PortTrials_North.ino
#       -Shape03_2-PortTrials_East.ino
#       -Shape03_2-PortTrials_South.ino
#       -Shape03_2-PortTrials_West.ino
...
maxCompletedTrialNum = 50                           #maximum number of trials to run
...
trainingProtocol = "Shape03_2-PortTrials"
...
while completedTrialNum < maxCompletedTrialNum:
    ...
    if initPortLocation == 'N':
        ...
        if len(irDetectorState) == 8:                               #length of "broken\n" (from arduino code)
            ...
            #pick next respone port location
            if random.randint(1,2) == 1:
                respPortLocation = 'E'
            ...
            if respPortLocation == 'E':
                while True:
                    ...
                    if len(irDetectorState) == 8:                                   #length of "broken\n" (from arduino code)
                        ...
                        interTrialCountdown = 5
                        print "Seconds til next trial"
                        while (interTrialCountdown > 0):
                            print interTrialCountdown
                            time.sleep(1)
                    ...
                    ##################################################
                        ...
                        #send east and west modules new port code
                        serEast.write(initPortLocation)
                        time.sleep(0.02)
                        serWest.write(initPortLocation)
                        time.sleep(0.02)
                        ...
                        #Countdown til next trial
                        interTrialCountdown = 60
                        print "\nNo Response"
                        print "Seconds til next trial"
                        while (interTrialCountdown > 0):
                            if (interTrialCountdown % 10 == 0):
                                print interTrialCountdown
                            time.sleep(1)
                            interTrialCountdown = interTrialCountdown-1
                        print "\n"
