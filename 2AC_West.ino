...
void loop()
{
  ...
  else if (incomingByte == 'W')     //'W' sent from python to activate West response port for West-stim trials
  {
    ...
    if (sensorState == 0)           //sensor beam is broken (LOW)
    {
      ...
      delay(rewardDelay);
      StepperMotorReward();         // dispense reward for west response on West-stim trial
  ...
  else if (incomingByte == 'E')      //'E' sent from python to activate West response port for East-stim trials
  {
    sensorState = digitalRead(SENSORPIN);  // check if the sensor beam is broken (i.e., LOW)
    ...
    if (sensorState == 0)
    {
      digitalWrite(LEDPIN, LOW);     //turn off LED
      Serial.println("Broken");      //"broken" is needed for python...which checks for string length
      //StepperMotorReward();        //no reward for West response on East-stim trial
      clearReadBuffer();            //1st clear buffer; this is necessary
      delay(interTrialInterval);
      clearReadBuffer();
    }
